...
describe("verifyAgentAction — valid decodes (1-11)", () => {
  it("1. tokenLock createLock", () => {
    ...
    expect(result.decoded.args).toEqual([
  ...
  it("4. tokenLock approve", () => {
    ...
    expect((result.decoded.args[0] as string).toLowerCase()).toBe(
      MPGR_TOKEN_LOCK_CONFIG.address.toLowerCase(),
    );
  });
  ...
  it("5. staking approve", () => {
    ...
    expect((result.decoded.args[0] as string).toLowerCase()).toBe(
      MPGR_STAKING_CONFIG.address.toLowerCase(),
    );
  });
...
describe("verifyAgentAction — mutation / regression (12-25)", () => {
  it("12. mutated `to` rejected", () => {
    ...
    const mutated: AgentActionContract = {
      ...action,
      to: ATTACKER as Address,
    };
  ...
  it("12b. malformed `to` never throws", () => {
    const action = mustBuild({
      domain: "staking",
      actionType: "stake",
      params: { amount: 100 },
    ...
    const nonString: AgentActionContract = {
      ...action,
      to: 12345 as unknown as Address,
    };
    ...
    if (!nonStringResult.ok) {
      expect(nonStringResult.error.code).toBe("INVALID_DESTINATION");
    }

    const malformedString: AgentActionContract = {
      ...action,
    ...
    if (!malformedResult.ok) {
      expect(malformedResult.error.code).toBe("INVALID_DESTINATION");
    }
  });

  it("13. mutated calldata selector rejected", () => {
    const action = mustBuild({
      domain: "staking",
      actionType: "stake",
      params: { amount: 100 },
  ...
  it("23. unsupported function rejected for tokenLock", () => {
    const action = mustBuild({
      domain: "tokenLock",
      actionType: "withdraw",
      params: { lockId: 1 },
  ...
  it("24. actionType/params mismatch rejected", () => {
    const action = mustBuild({
      domain: "staking",
      actionType: "stake",
      params: { amount: 100 },
    ...
    const malformed = {
      ...
      domain: "staking",
      actionType: "createLock",
      params: { actionType: "createLock" },
    } as unknown as AgentActionContract;
  ...
  it("24b. envelope actionType divergence rejected", () => {
    const withdrawAction = mustBuild({
      domain: "tokenLock",
      actionType: "withdraw",
      params: { lockId: 4 },
    });
  ...
  it("25. calldata/params mismatch rejected", () => {
    const a = mustBuild({
      domain: "staking",
      actionType: "stake",
      params: { amount: 100 },
...
describe("simulateAgentAction — read-only simulation (26-28)", () => {
  it("26. simulation failure returns typed failure", async () => {
    mockSimulateContract.mockRejectedValueOnce(
      new Error("execution reverted: some raw provider detail"),
    );
  ...
  it("27. missing account produces typed failure and never simulates", async () => {
    const action = mustBuild({
      domain: "staking",
      actionType: "stake",
      params: { amount: 100 },
  ...
  it("27b. verification failure short-circuits simulation", async () => {
    const action = mustBuild({
      domain: "staking",
      actionType: "stake",
      params: { amount: 100 },
    ...
    const result = await simulateAgentAction(mutated, {
      account: SOME_ACCOUNT,
    });
