...
const { x402DiscoverResourceTool, x402PreparePaymentTool } = await import("../x402-tool-definitions");
...
describe("x402 tool shapes", () => {
  ...
  it("34. the side-effect import registers both tools into the real production registry singleton", () => {
    ...
    expect(registry.get("x402_discover_resource")).toBeDefined();
    expect(registry.get("x402_prepare_payment")).toBeDefined();
  });
});
...
describe("x402 tools via AgentToolRuntime", () => {
  it("35. x402_discover_resource runs through the runtime and reports a payment requirement", async () => {
    ...
    expect((result.data as { paymentRequired: boolean }).paymentRequired).toBe(true);
    vi.unstubAllGlobals();
  });
  ...
  it("36. x402_discover_resource reports paymentRequired: false for a non-402 resource", async () => {
    ...
    expect(result.success).toBe(true);
    expect((result.data as { paymentRequired: boolean }).paymentRequired).toBe(false);
    vi.unstubAllGlobals();
  });
  ...
  it("37. x402_prepare_payment returns a structured proposal, never a signature or submission", async () => {
    ...
    expect(result.success).toBe(true);
    const data = result.data as { proposal: { requiresConfirmation: boolean; phase: string; id: string } };
    expect(data.proposal.requiresConfirmation).toBe(true);
    expect(data.proposal.phase).toBe("idle");
    vi.unstubAllGlobals();
  });
  ...
  it("39. calling a hypothetical execute-mode x402 tool id fails at lookup — it was never registered", async () => {
    const runtime = makeRuntime();
    const result = await runtime.executeTool("x402_execute_payment", {}, {});
    expect(result.success).toBe(false);
  ...
  it("40. rejects an invalid resourceUrl before ever calling fetch", async () => {
    ...
    const result = await runtime.executeTool("x402_discover_resource", { resourceUrl: "not-a-url" }, {});
