// app/api/trade/price/route.test.ts
//
...
// reached past the origin gate, without needing the trade-router/quote
...
function postPrice(headers: Record<string, string> = {}) {
  return new Request(`${APP_ORIGIN}/api/trade/price`, {
    method: "POST",
...
describe("POST /api/trade/price — CSRF (real verifyTrustedOrigin)", () => {
  ...
  it("lets a trusted-origin request reach the route's own logic (past the origin gate)", async () => {
    ...
    const response = await POST(postPrice({ origin: APP_ORIGIN }));
  ...
  it("rejects a cross-origin request with 403 before the protected operation runs", async () => {
    const { POST } = await import("./route");
    const response = await POST(postPrice({ origin: "https://evil.com" }));
  ...
  it("rejects a request with no Origin/Referer at all", async () => {
    const { POST } = await import("./route");
    const response = await POST(postPrice());
    expect(response.status).toBe(403);
    expect(getSessionFromRequest).not.toHaveBeenCalled();
  });
});
