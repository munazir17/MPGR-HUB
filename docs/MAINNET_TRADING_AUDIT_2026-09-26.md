...
**The browser trading flow and MCP Executor flow are different existing paths.** The browser still uses its existing CDP/0x/Aerodrome provider selection and separate post-swap wallet-confirmed fee transfer. MCP uses the Executor's atomic fee for its proven pair. Migrating the UI would change architecture/routing/fee collection and was deliberately not done. A genuine in-app Aerodrome quote is not falsely relabeled Uniswap. The modal explains the separate MCP route for USDC/WETH.
